        ...
        with tf.name_scope('edge_feature'):
            ...
            def py_canny(image_tf_input): # Renamed arg to avoid confusion, it's a TF Tensor initially
                try:
                    ...
                    if image_np.shape != (global_config.image_size, global_config.image_size):
                        # Resize if shape doesn't match
                        image_np = cv2.resize(image_np, (global_config.image_size, global_config.image_size))

                    # Apply Canny, thresholds can be tuned or made dynamic
                    edges = cv2.Canny(image_np, threshold1=100, threshold2=200)
                    
                    # Ensure output shape is correct
                    if edges.shape != (global_config.image_size, global_config.image_size):
                        edges = cv2.resize(edges, (global_config.image_size, global_config.image_size))
                    
                    # Normalize to 0-1
                    result = edges.astype(np.float32) / 255.0
                    
                    # Final shape guarantee
                    if result.shape != (global_config.image_size, global_config.image_size):
                        result = np.full((global_config.image_size, global_config.image_size), 0.0, dtype=np.float32)
                    ...
                    return np.full((global_config.image_size, global_config.image_size), 0.0, dtype=np.float32)

            edge_map = tf.py_function(py_canny, [image_tf], tf.float32)
            edge_map.set_shape(tf.TensorShape([global_config.image_size, global_config.image_size])) # Set shape info for graph compilation
        ...
        with tf.name_scope('symmetry_feature'):
            ...
            def py_radon_symmetry(gray_tf_input): # Renamed arg to avoid confusion, it's a TF Tensor initially
                try:
                    ...
                    expected_shape = (global_config.image_size, global_config.image_size)
            ...
            feature_map.set_shape(tf.TensorShape([global_config.image_size, global_config.image_size])) # Set exact shape info
