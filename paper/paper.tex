...
% Relax strict LaTeX float constraints to prevent small tables/figures from taking up entire pages
...
\end{equation}
The gradient magnitude $M = \sqrt{G_x^2 + G_y^2}$ is computed per pixel. The base feature score is the inverse of the normalized magnitude variance over the patch: $P_{\text{grad}} = 1 / (\text{Var}(M / G_{\text{max}}) + \epsilon)$. To incorporate spatial weighting, this base patch score is multiplied by a centered radial decay map $g(y,x)$ computed for each pixel coordinate $(y,x)$ within the patch:
\begin{equation}
g(y,x) = 1.0 - \frac{\sqrt{(y - y_c)^2 + (x - x_c)^2}}{d_{\text{max}}}
\label{eq:radialdecay}
\end{equation}
where $(y_c, x_c)$ is the pixel-level patch center and $d_{\text{max}}$ is the maximum distance from the center to a corner within the $8 \times 8$ pixel patch. The final feature value for the patch is the mean of this weighted pixel-level map. Unnaturally smooth transitions in AI-generated images produce lower gradient variance, yielding higher perfection scores.
...
\end{equation}
A low variance of the Laplacian response indicates an unnaturally smooth noise distribution, characteriztic of generated images, as opposed to the irregular grain introduced by natural camera sensors; a high variance indicates the noisy, textured response typical of authentic photographs.

The symmetry feature measures horizontal and vertical pixel-difference symmetry. It splits the patch vertically and horizontally, reversing one half to align coordinates. Horizontal and vertical symmetry scores are computed as:
\begin{equation}
S_{\text{h}} = 1.0 - \frac{\text{Mean}(|I_{\text{left}} - \text{Rev}(I_{\text{right}})|)}{\max(I) + \epsilon}, \quad S_{\text{v}} = 1.0 - \frac{\text{Mean}(|I_{\text{top}} - \text{Rev}(I_{\text{bottom}})|)}{\max(I) + \epsilon}
\end{equation}
The composite symmetry score $(S_{\text{h}} + S_{\text{v}}) / 2$ is multiplied by the same centered radial decay map $g(y,x)$ from Eq.~\ref{eq:radialdecay} to produce a pixel-level weighted map, whose mean over the patch pixels is taken as the final feature value. Generative models, particularly those conditioned on symmetric prompts, exhibit bias toward unnatural levels of spatial symmetry.
...
\end{equation}
The feature value is the mean normalized LBP code over the patch, capturing micro-texture regularities.

The color saturation feature converts the patch to HSV color space and computes the variance of the saturation channel:
\begin{equation}
\sigma_S^2 = \frac{1}{HW} \sum_{i,j} (S_{ij} - \mu_S)^2
\end{equation}
AI-generated images often exhibit unnaturally uniform color distributions that produce low saturation variance.

The hash complexity feature computes a DCT-based perceptual hash by reducing the patch to an $8 \times 8$ grayscale representation, computing the 2D Discrete Cosine Transform, and binarising the coefficients relative to their mean. The feature value is the normalized count of bit transitions along rows and columns, capturing structural order.

The GLCM texture feature computes a Gray-Level Co-occurrence Matrix~\cite{haralick1973textural} approximation. The patch intensities are quantized to 16 levels, and the co-occurrence matrix is computed for a horizontal displacement ($\Delta x = 1, \Delta y = 0$). The patch score is a linear combination of normalized contrast $C_{\text{norm}}$ and homogeneity $H_{\text{norm}}$:
\begin{equation}
\text{GLCM}_{\text{score}} = 0.6 \cdot H_{\text{norm}} + 0.4 \cdot (1 - C_{\text{norm}})
\end{equation}
...
Feature values are dynamically normalized using per-feature 99th-percentile scales computed from the training set, ensuring consistent ranges across datasets and preventing outlier domination. These scales are computed once during the preprocessing stage and serialised for consistent application during evaluation and inference. Crucially, scaling the features into the unit interval $[0, 1]$ (with values above the 99th percentile clipped to 1.0) enables a uniform ruleset representation in the genetic algorithm. Because every feature channel is bounded to $[0, 1]$, the GA can utilize a single, uniform threshold search space of $[0, 1]$ and uniform crossover/mutation operators across all rules, regardless of the physical units or raw magnitudes of the underlying features (e.g., Laplacian variance vs.\ Local Binary Patterns).
...
Each individual in the population encodes a variable-length set of conditional rules (up to 40 rules, with typically 10 active at initialisation) as a tensor of shape $[\text{max\_rules}, 4]$, where each row encodes a rule $r_i = [f_i, \theta_i, op_i, a_i]$ representing feature index, threshold, operator, and action ($a_i \in \{\text{include}, \text{exclude}\}$), respectively. The feature index selects one of the eight feature channels, the threshold is a scalar in $[0, 1]$, the operator is either greater-than or less-than, and the action is either include or exclude. Rules with a feature index of $-1$ are treated as inactive padding.

For a given image with patch features $\mathbf{F} \in \mathbb{R}^{32 \times 32 \times 8}$, each rule votes independently on each patch. Let $\mathbb{1}[r_i \text{ fires at } (h,w)]$ be an indicator variable representing whether rule $r_i$ is activated:
\begin{equation}
\mathbb{1}[r_i \text{ fires at } (h,w)] = \begin{cases} 
1 & \text{if } \mathbf{F}(h,w,f_i) \mathbin{op_i} \theta_i \\
0 & \text{otherwise} 
\end{cases}
\end{equation}
The final soft mask is computed as the difference of normalized include and exclude vote fractions:
\begin{equation}
m(h,w) = \text{clip}\!\left(V_{\text{incl}}(h,w) - V_{\text{excl}}(h,w),\; 0,\; 1\right)
\label{eq:softmask}
\end{equation}
where the vote fractions are aggregated as:
\begin{equation}
\begin{aligned}
V_{\text{incl}}(h,w) &= \frac{\sum_{r \in R_{\text{incl}}} \mathbb{1}[r \text{ fires at } (h,w)]}{\max(|R_{\text{incl}}|, 1)} \\[1ex]
V_{\text{excl}}(h,w) &= \frac{\sum_{r \in R_{\text{excl}}} \mathbb{1}[r \text{ fires at } (h,w)]}{\max(|R_{\text{excl}}|, 1)}
\end{aligned}
\end{equation}
and $R_{\text{incl}}$ and $R_{\text{excl}}$ denote the active subsets of include and exclude rules respectively. If a subset is empty, its vote fraction is set to zero. This formulation produces a soft mask: patches with net include votes receive fractional weights, and patches with net exclusion receive weight 0.0. The entire mask generation procedure is implemented as a single vectorised TensorFlow graph via \texttt{@tf.function} to allow batched processing on GPU without Python-level loops.
...
The divergence score $D$ is the primary fitness component and measures how well the mask-selected patches separate AI-generated from human-generated images across the evaluation batch. Because the statistical distance measures (FDR and Bhattacharyya) are highly sensitive to class size imbalances, the pipeline enforces strict stratified sampling to maintain a $50/50$ AI-to-human class layout during evaluation. This prevents the divergence metric from being biased by majority categories or specific generator subsets. The divergence score combines Fisher Discriminant Ratio (FDR)~\cite{fisher1936use} and Bhattacharyya distance~\cite{bhattacharyya1943measure} into a composite score weighted by the per-feature importance weights from Phase~1:
\begin{equation}
D = \sum_f w_f \left(0.6 \cdot (1 - e^{-\text{FDR}_f}) + 0.4 \cdot (1 - e^{-\text{BC}_f})\right)
\label{eq:divergence}
\end{equation}
The FDR for each feature channel $f$ is computed as:
\begin{equation}
\text{FDR}_f = \frac{(\mu_{f}^{\text{ai}} - \mu_{f}^{\text{human}})^2}{\sigma_{f,\text{ai}}^2 + \sigma_{f,\text{human}}^2 + \epsilon}
\end{equation}
where $\mu$ and $\sigma^2$ are the mean and variance of the masked feature vectors for each class. The Bhattacharyya distance uses a Gaussian approximation combining both shape and Mahalanobis terms:
\begin{equation}
\text{BC}_f = 0.5 \ln\!\left(\frac{\bar{\sigma}_f^2}{\sigma_{f,\text{ai}} \sigma_{f,\text{human}}}\right) + 0.25 \frac{(\mu_f^{\text{ai}} - \mu_f^{\text{human}})^2}{\sigma_{f,\text{ai}}^2 + \sigma_{f,\text{human}}^2}
\end{equation}
Both measures are saturated through the $1 - e^{-x}$ transform to prevent unbounded growth. To prevent degenerate masks that select only a tiny fraction of patches and overfit to the training batch without generalizing, the divergence score is gated by a sparsity floor: $D_{\text{gated}} = D \cdot \min(\text{sparsity}/0.10, 1)$. This gates out degenerate masks selecting fewer than 10\% of the spatial grid.
...
\end{equation}
where $t$ is the target sparsity and $r$ is the tolerance radius. This produces a plateau of perfect efficiency within the target range and smooth degradation outside it, preventing the search from collapsing into degenerate all-mask (which mimics the baseline CNN) or zero-mask solutions.

The connectivity score $C$ measures spatial coherence of selected patches using convolution with a cross-shaped 3$\times$3 kernel. For each selected patch, the kernel counts the number of selected neighbours (up to 4). The connectivity score is the mean ratio of actual to maximum possible connections across the batch, normalized to $[0, 1]$. This cross-shaped kernel prior rewards spatial contiguity of selected patches, encouraging the GA to select contiguous physical regions and image boundaries carrying actual generative artifacts, rather than scattered, high-frequency "salt-and-pepper" noise patches that lack structural coherence.

The simplicity score $S$ penalises rule complexity beyond the number of features: $S = 1 - \max(\text{active\_rules} - 8, 0) / (\text{max\_rules} - 8)$. This parsimony constraint prevents the GA from overfitting through excessive rule counts and rule bloat, forcing the search to evolve compact, interpretable conditional rulesets.

An inactive-weight penalty is applied to individuals that ignore high-weight features. The penalty sums the Phase~1 weights of features not utilised by any active rule and is phased in proportionally to the divergence score to avoid suppressing immature but promising individuals early in evolution. This enforces an "Honest Weights" policy, preventing the GA from ignoring statistically validated feature domains (e.g., noise uniformity or gradient perfection) in favor of exploiting noisy, low-weight features to achieve false statistical divergence.
...
To prevent overfitting during the evolutionary search, the genetic algorithm incorporates a validation checkpointing mechanism utilizing a separate probe dataset. At designated generations, the best individual in the population is evaluated on the probe set. A blended validation score is computed as a weighted combination of training batch fitness ($30\%$) and probe set fitness ($70\%$):
\begin{equation}
\text{Fitness}_{\text{val}} = 0.3 \cdot \text{Fitness}_{\text{train}} + 0.7 \cdot \text{Fitness}_{\text{probe}}
\label{eq:valblend}
\end{equation}
The candidate ruleset that achieves the highest validation score across all generations is restored as the final evolved ruleset. This blending forces the genetic search to prioritize rulesets that maintain high discriminability on the training set while demonstrating generalizability on the probe set.

\subsection{Random Mask Generator (Control)}

The random mask control is implemented as a subclass of the feature pipeline that replaces the GA mask generation step with a stochastic process. For each image, each patch is independently selected with probability equal to the mean sparsity of the GA masks (approximately 16.4\%). Seeds are derived deterministically from per-image feature sums via \texttt{tf.random.stateless\_uniform} to ensure that different images receive different masks while maintaining reproducibility across runs. The random pipeline reuses all other infrastructure---feature extraction, caching, normalization, and dataset preparation---identically to the GA pipeline, ensuring that the only difference between the two conditions is the structure of the mask.
\subsection{CNN Architecture}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.45\textwidth]{model_architecture}
\caption{Detailed architecture of the proposed CNN model with attention fusion. The image branch and feature branch process the raw image and upsampled feature map in parallel. A $1 \times 1$ convolution gates the spatial feature activations before fusion via cross-attention and a residual addition back into the image classification stream.}
\label{fig:architecture}
\end{figure}

All three model variants share the same CNN architecture, implemented using TensorFlow and Keras~\cite{tensorflow2015whitepaper}. The image branch consists of three convolutional blocks, each containing a 2D convolutional layer with $3 \times 3$ kernels (32, 64, and 128 filters respectively), ReLU activation, $2 \times 2$ max-pooling, and batch normalization~\cite{ioffe2015batch}.

For the GA-masked and random-masked models, a parallel feature branch processes the $256 \times 256 \times 8$ feature map (mask-weighted patch features upsampled from the $32 \times 32$ patch grid to image resolution via nearest-neighbour interpolation) through a mirrored three-block architecture with the same filter progression. A $1 \times 1$ convolutional attention gate on the image branch produces spatial weights in $[0, 1]$ that modulate the feature branch output via element-wise multiplication. The modulated features are then element-wise multiplied with the image branch activations (cross-attention) and added back to the image branch through a residual connection. This mechanism allows the CNN to selectively integrate information from the mask-weighted features where the image content suggests relevance. The detailed model architecture, including attention fusion, is shown in Fig.~\ref{fig:architecture}.

After the attention fusion, a fourth convolutional layer (128 filters, $3 \times 3$) feeds into $2 \times 2$ max-pooling, batch normalization, and global average pooling. Two dropout layers with rates determined by hyperparameter optimization precede a dense layer and a sigmoid output. The baseline model replaces the attention fusion with an additional convolutional layer to maintain comparable model capacity. Training uses the Adam optimizer~\cite{kingma2015adam} with binary cross-entropy loss, mixed-precision arithmetic (float16 compute, float32 accumulation) via \texttt{LossScaleOptimizer} for GPU efficiency, learning rate reduction on plateau, and early stopping. L2 kernel regularisation is applied to all convolutional and dense layers.
...
Three generators are used for training: ADM~\cite{dhariwal2021diffusion}, GLIDE~\cite{nichol2022glide}, and Wukong. From each generator, 10{,}000 AI-generated and 10{,}000 real images are sampled, yielding a balanced training set of 30{,}000 images. For in-distribution evaluation, a stratified 20\% held-out validation split is drawn from the same generators (5{,}000 images). For cross-generator evaluation, two unseen generators are used: Stable Diffusion v4 (sdv4) and VQDM, with 2{,}500 AI-generated and 2{,}500 real images per generator (5{,}000 images total). All images are centre-cropped and resized to $256 \times 256$ pixels and normalized to $[0, 1]$. The dataset is loaded through a custom CSV-indexed pipeline built on TensorFlow's \texttt{tf.data} API, with stratified sampling to maintain balanced class distributions throughout all processing stages.

\subsection{Hyperparameter Optimization Framework and Objectives}

To systematically optimize the proposed pipeline, hyperparameter optimization (HPO) was conducted in three sequential phases using the Optuna framework~\cite{akiba2019optuna}: Phase~1 optimizes feature importance weights; Phase~2 tunes genetic algorithm parameters; and Phase~3 optimizes CNN training and architectural configurations.

To find parameters and feature weights that generalize robustly under distribution shift, Phases~1 and 2 utilize a validation checkpointing objective that blends training set performance with cross-generator probe set performance. The probe set consists of images from unseen generators (specifically, Stable Diffusion v4 and VQDM), allowing the search to directly target cross-generator generalizability. In Phase~1 (feature weight optimization), the trial objective is a $30/70$ blend of training and probe set fitness:
\begin{equation}
\text{Objective}_{\text{HPO}} = 0.3 \cdot \text{Fitness}_{\text{train}} + 0.7 \cdot \text{Fitness}_{\text{probe}}
\end{equation}
where both terms incorporate the inactive feature weight penalty.

In Phase~2 (GA hyperparameter optimization), the trial objective uses the same $30/70$ blend of unpenalized training and probe fitness, collapsing to pure probe fitness when the active weight penalty itself is being optimized to prevent numeric collapse of the search.

For Phase~3, CNN classifier hyperparameters were optimized targeting downstream cross-generator validation accuracy on the probe set. The search spaces and resulting parameters are detailed in the following sections.

\subsection{Phase 1 and Phase 2 Optimization Results}

Phase~1 determines the relative importance of each feature channel for separating AI-generated from human-generated images. An Optuna study optimizes the set of eight feature weights by maximizing the fusion fitness score, which measures the overall discriminability of the weighted feature combination. Table~\ref{tab:feature_weights} presents the learned weights along with the achieved fusion fitness score. The corresponding fitness function component weights used in the genetic algorithm are listed in Table~\ref{tab:fitness_weights}.

\begin{table}[!htbp]
\begin{minipage}[t]{0.48\textwidth}
\centering
\caption{Learned feature weights from Phase~1 Bayesian optimization.}
...
Noise uniformity receives the highest weight (0.318), consistent with the well-established observation that generative models produce unnaturally uniform noise distributions compared to natural camera sensor noise~\cite{liu2022detecting}. Gradient perfection ranks second (0.220), capturing the overly smooth gradient transitions characteriztic of diffusion model outputs. Hash complexity and symmetry contribute moderately, while GLCM texture and color saturation contribute minimally, suggesting that co-occurrence statistics and color distributions are less discriminative at the $8\times 8$ patch scale used in this study.

The four components of the GA fitness function (Eq.~\ref{eq:fitness}) are weighted as shown in Table~\ref{tab:fitness_weights}. The divergence score receives the dominant weight (0.75), reflecting the primary objective of maximizing inter-class feature separation. Efficiency and simplicity each receive a weight of 0.10 to prevent degenerate or overly complex solutions, while connectivity receives a minimal weight of 0.05 as a soft encouragement toward spatially coherent masks.

For Phase~2, the GA hyperparameters selected through Bayesian optimization are detailed in Table~\ref{tab:ga_params}. Under this setup, a population of 120 individuals evolves for 80 generations across three independent runs, and the candidate ruleset achieving the highest validation-checkpointed fitness is selected for downstream classification in Phase~3.

\begin{table}[!htbp]
\centering
\caption{Genetic algorithm hyperparameters. Values were selected through Bayesian optimization.}
\label{tab:ga_params}
\begin{tabular}{@{}lc@{}}
\toprule
Parameter & Value \\
...
\subsection{Phase 3: CNN Classifier Optimization Results}

CNN hyperparameters were optimized for the baseline and GA mask modes through separate Optuna studies targeting cross-generator validation accuracy. The random mask model reuses the GA mask hyperparameters to ensure a controlled comparison where the network architecture and training regime are identical, isolating the effect of mask structure. Table~\ref{tab:cnn_params} presents the resulting configurations.

\begin{table}[!htbp]
\centering
\caption{CNN hyperparameters per mask mode. Values were selected through Bayesian optimization targeting cross-generator validation accuracy.}
...
Fig.~\ref{fig:training_indist} shows the training history for the baseline model in the in-distribution setting. Training accuracy converges smoothly toward 99\%, while validation accuracy exhibits characteriztic oscillation around 90--97\% due to the relatively small validation set and the batch normalization statistics mismatch between training and evaluation modes. The gap between training and validation loss indicates moderate overfitting, which is mitigated by early stopping.
...
The GA-masked model achieves the highest overall accuracy (78.64\%) compared to the baseline (77.68\%) and random masking (77.50\%). The margin is approximately one percentage point over the baseline, which is modest but aligns with our claim that structured masking provides additional discriminative signal under distribution shift.

The three models exhibit markedly different precision--recall trade-offs under shift, which explains the apparent discrepancy between overall accuracy and F1 score. In a balanced binary classification task, accuracy is the arithmetic mean of sensitivity (recall) and specificity (True Negative Rate). The GA-masked model achieves a significantly higher specificity (91.84\%) than the baseline (86.24\%), correctly classifying 2,296 authentic human-generated images compared to the baseline's 2,156. This represents a 40.7\% reduction in False Positives (from 344 to 204). The random-masked model represents the extreme end of this trade-off, achieving 97.68\% specificity (only 58 False Positives) but failing to detect 42.68\% of AI-generated images (1,067 False Negatives). 

The F1 score, which is mathematically insensitive to True Negatives, is slightly lower for the GA-masked model than the baseline (75.39\% vs.\ 75.59\%) because the sparse masking channel (which mutes approximately 83.6\% of the spatial feature grid) naturally limits the density of synthetic artifacts available to the CNN, biasing the network toward conservatism (sensitivity of 65.44\% vs.\ 69.12\% for the baseline). However, in real-world deployment contexts (e.g., flagging fake news or checking digital evidence), misclassifying authentic user media as AI-generated carries severe operational and trust penalties, making the GA model's precision boost (88.91\% vs.\ 83.40\% for the baseline) a highly favorable trade-off.

The random-masked model performs at or slightly below the baseline in both accuracy (77.50\% vs.\ 77.68\%) and F1 score (71.81\% vs.\ 75.59\%). This result confirms that the GA mask's advantage is not simply due to sparsity-induced regularisation or the provision of an extra input channel. The specific structure of patch selection is what matters.

The contrast between in-distribution and cross-generator results is informative. In-distribution, the GA mask's advantage over random masking is negligible (less than 0.1\% accuracy). Under cross-generator shift, the gap grows to approximately 1.14\% in accuracy and 3.58\% in F1 score. This pattern supports our claim that the advantage of structured masking is more pronounced under distribution shift.
...
The current configuration reflects deliberate engineering and experimental decisions, each made under computational budget constraints. The specific generator split—training on ADM, GLIDE, and Wukong while testing generalization on Stable Diffusion and VQDM—was selected to ensure representational coverage across three distinct architectural families: pixel-space diffusion, latent diffusion, and vector-quantized autoregressive structures. While testing under different generator permutations would naturally yield varying absolute accuracy margins (an avenue left for future validation work), this specific setup represents the most structurally comprehensive baseline to probe.

The 8$\times$8 pixel patch size was chosen to balance spatial granularity against the dimensionality of the feature tensor and the computational cost of per-patch feature extraction across thousands of images. At this resolution, generator artifacts that manifest at larger spatial scales---such as anatomical inconsistencies in faces and hands---fall outside the scope of patch-level features. The eight handcrafted features were selected to cover complementary signal domains (spatial gradients, noise statistics, texture, color, frequency), though they represent a subset of the possible discriminative signals available in each patch. The GA fitness function optimizes for statistical feature divergence rather than downstream CNN accuracy; this was a deliberate decoupling that allows the GA to operate independently of the CNN training loop, substantially reducing the computational cost of the pipeline. The stochastic nature of genetic algorithms is a well-known consideration, addressed in our implementation through multiple independent GA runs, Hall of Fame elitism with similarity-based deduplication, and an inactive-weight penalty that encourages broad feature utilisation across the evolved rules.

The structured nature of the evolved masks (discussed in Section~5.4) validates the fitness function's constraint design. Without the sparsity floor gating, parsimony constraints, and spatial priors, the GA could have easily converged to degenerate shortcuts---such as selecting a tiny fraction of highly overfitted patches or using bloated rulesets to fit the training batch. The average mask coverage of 16.4\% and the contiguity of the selected regions (Fig.~\ref{fig:mask_comparison}) confirm that the combination of the quadratic efficiency score, the simplicity penalty, and the cross-shaped connectivity prior successfully guided the evolutionary search toward robust, physically meaningful patch-selection strategies. This confirms that guiding a deep neural network through evolved, handcrafted statistical priors is a viable alternative to pure end-to-end representation learning under distribution shift.

The feature weight analysis in Table~\ref{tab:feature_weights} reveals that noise uniformity and gradient perfection together account for more than half of the total feature weight (53.8\%). The lower-weight features (GLCM at 1.4\%, color saturation at 3.4\%) contribute minimally to the GA's mask construction, indicating that the discriminative signal at this patch scale is concentrated in noise and gradient statistics rather than in color or co-occurrence texture measures.

The pipeline encompasses a large number of interacting components---patch size, feature set, GA hyperparameters, CNN architecture, attention mechanism design, and fitness function formulation---each optimized within the available computational budget using Bayesian hyperparameter search, though exhaustive ablation across all combinations was not feasible. The results demonstrate that the general approach of using evolutionary search to construct per-image attention masks for a CNN produces measurable benefits under distribution shift, and provide a foundation for further investigation.

Future work could extend the approach in several directions. Multi-scale patch extraction would allow the system to capture artifacts that span larger spatial regions. Replacing handcrafted features with learned representations from a pretrained backbone (e.g., CLIP or DINOv2) would provide richer feature spaces for the GA to operate on. Aligning the GA fitness function with downstream CNN accuracy through a bilevel optimization formulation would close the gap between mask quality and classification performance. Finally, compression-aware feature extraction---for example, computing features in a JPEG-invariant representation---could mitigate the compression fragility observed in our experiments.

\subsection{Engineering Context and Ethical Considerations}

Several engineering constraints shaped the design of this system. The computational cost of the pipeline was a primary concern: per-patch feature extraction across tens of thousands of images, combined with a population-based evolutionary search over 80 generations, required careful budgeting of GPU compute time. This motivated the decoupled architecture in which the GA operates on precomputed feature statistics independently of the CNN training loop, and the use of mixed-precision arithmetic and NumPy memory-mapped caching to reduce memory pressure and accelerate data throughput. Reproducibility was addressed through fixed random seeds, deterministic mask generation via stateless random operations, serialisation of all hyperparameters and feature normalization scales, and systematic logging of all experiment configurations and results.

From an ethical and social perspective, the detection of AI-generated images is directly relevant to combating visual misinformation, identity fraud, and non-consensual synthetic media. A detector that generalizes across generator families---the central goal of this work---is more practically useful in these contexts than one that only identifies images from known sources. At the same time, detection tools carry dual-use risk: the same feature-level analysis that identifies synthetic content could theoretically be used to refine generators to evade detection. The interpretability of the evolved rulesets, discussed in Section~5.4, provides transparency into what the system considers discriminative, which supports accountability in deployment contexts.
